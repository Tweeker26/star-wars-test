<template>
  <div class="tooltip">
    <div v-if="text" class="tooltip__dot"></div>
    <div class="popup">
      <textarea
        v-model="text"
        class="popup__text"
      ...
      <div class="popup__buttons">
        <button v-if="text && edited" class="popup__btn" @click="handleSave">
          <Icon name="tick1" height="15" width="15" />
        </button>
        <button v-if="text" class="popup__btn" @click="handleDelete">
          <Icon name="cancel" height="12" width="12" />
        </button>
      </div>
    </div>
    <div ref="mySlot" class="tooltip__content">
      <slot></slot>
    </div>
  </div>
</template>
